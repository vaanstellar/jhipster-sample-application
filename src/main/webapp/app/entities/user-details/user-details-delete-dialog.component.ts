...
export class UserDetailsDeletePopupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activatedRoute.data.subscribe(({ userDetails }) => {
      setTimeout(() => {
        this.ngbModalRef = this.modalService.open(UserDetailsDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
        this.ngbModalRef.componentInstance.userDetails = userDetails;
        this.ngbModalRef.result.then(
          result => {
            this.router.navigate(['/user-details', { outlets: { popup: null } }]);
